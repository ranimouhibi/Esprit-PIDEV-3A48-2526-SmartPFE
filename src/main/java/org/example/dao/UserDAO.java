...
public class UserDAO {
    ...
    public List<User> findByEstablishment(int establishmentUserId) throws SQLException {        List<User> users = new ArrayList<>();
        // Find users whose establishment_id matches the establishment user's establishment_id
        String sql = "SELECT u.* FROM users u " +
                     "JOIN users est ON est.id = ? " +
                     "WHERE u.establishment_id = est.establishment_id AND u.id != ? ORDER BY u.name";
        try (PreparedStatement ps = DatabaseConfig.getConnection().prepareStatement(sql)) {
            ps.setInt(1, establishmentUserId);
            ps.setInt(2, establishmentUserId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) users.add(mapRow(rs));
        }
        // Fallback: direct match on establishment_id = establishmentUserId
        if (users.isEmpty()) {
            String sql2 = "SELECT * FROM users WHERE establishment_id = ? ORDER BY name";
            try (PreparedStatement ps = DatabaseConfig.getConnection().prepareStatement(sql2)) {
                ps.setInt(1, establishmentUserId);
                ResultSet rs = ps.executeQuery();
                while (rs.next()) users.add(mapRow(rs));
            }
    ...
    public void updateProfile(User user) throws SQLException {
        // Base update  always works regardless of whether profile_picture column exists
        ...
        // Store Cloudinary URL  also handles null (clears picture), skip if column missing
        try {
            ...
            // Column doesn't exist yet  run: ALTER TABLE users ADD COLUMN profile_picture VARCHAR(500) NULL
        }
    }
