...
public class MeetingDAO {
    ...
    /** Returns true if a meeting already exists on the same calendar date (excluding the given id for edits, pass -1 for new). */
    public boolean existsByDate(java.time.LocalDate date, int excludeId) throws SQLException {
        String sql = "SELECT COUNT(*) FROM meetings WHERE DATE(scheduled_date) = ? AND id != ?";
        try (PreparedStatement ps = DatabaseConfig.getConnection().prepareStatement(sql)) {
